...
public class MercadoLibre implements Portal {
...
/*    	if (urlLog4j != null) {
        ...
        if (tipoPropiedad.equals("Casa")){
        	urlParams = urlParams+"casas/rm-metropolitana";
        }else{
        	urlParams = urlParams+"departamentos/rm-metropolitana";
        }
        if (tipoTransaccion.equals("Venta")){
        	urlParams = urlParams+"-venta";
        }else{
        	this.path =this.path+"\\Arriendo"; 
        	urlParams = urlParams+"-arriendo";
        }
/*		if (!dormitorios.trim().isEmpty() && !dormitorios.equals("Seleccione")){
			Integer dorm = Integer.parseInt(dormitorios.trim());
			if (dorm <=3){
				urlParams = urlParams+dorm+"-dormitorio";
			}else{
				urlParams = urlParams+"4-dormitorios-o-mas";	
			}
        ...
        if (moneda.equals("UF")) {
			...
			if (!precioDesde.trim().isEmpty() && !precioHasta.trim().isEmpty()) {
				priceRange = priceRange + "_PriceRange_"+precioDesde.trim()+"CLF-"+precioHasta.trim()+"CLF";
			} else if (!precioDesde.trim().isEmpty() && precioHasta.trim().isEmpty()) {
				priceRange = priceRange + "_PriceRange_"+precioDesde.trim()+"CLF-0CLF";
			} else if (precioDesde.trim().isEmpty() && !precioHasta.trim().isEmpty()) {
				priceRange = priceRange + "_PriceRange_0CLF-"+precioHasta.trim()+"CLF";
			}
			
		}else{
			
			if (!precioDesde.trim().isEmpty() && !precioHasta.trim().isEmpty()) {
				priceRange = priceRange + "_PriceRange_"+precioDesde.trim()+"CLP-"+precioHasta.trim()+"CLP";
			} else if (!precioDesde.trim().isEmpty() && precioHasta.trim().isEmpty()) {
				priceRange = priceRange + "_PriceRange_"+precioDesde.trim()+"CLP-0CLP";
			} else if (precioDesde.trim().isEmpty() && !precioHasta.trim().isEmpty()) {
				priceRange = priceRange + "_PriceRange_0CLP-"+precioHasta.trim()+"CLP";
			}
        ...
        for(int x = 1; x<=MaxPaginas;x++){
        	try{
        		if (x > 1){
        			pag = pag+48;
        			pags = "_Desde_"+pag;
        			//urlParams = urlParams.replace("/venta/", "-venta");
        		}else{
        			pags = "";
        			
        		}
        		//System.out.println("URL: "+url);
        		Document doc = Jsoup.connect(url+urlParams+pags+priceRange).userAgent(userAgent).timeout(timeout).get();
				...
				System.out.println("Scraping page: "+tipoPropiedad+" "+tipoTransaccion+" MercadoLibre.cl "+x+" of "+MaxPaginas);
        		for(int i = 1; i<=50; i++){
        			orden++;
					...
					if (id != null && !id.trim().isEmpty()) {
						try {
							...
							System.out.println("#"+ id.trim()+ " > div.item__info-container > div > h2 > a > span");
							System.out.println("Url: " + strUrl);

							e.printStackTrace();
						}
						if (id.trim().equals("MLC458777695")){
							System.out.println();
						}
						...
						if (strName.isEmpty()) {
							System.out.println("Pagina: _Desde_" + pag);
        ...
        System.out.println("MercadoLibre Proceso terminado");
	...
	private String getDormitorios(String str){
		String result = "";
		if (str != null){
			if (str.contains("totales") && !str.contains("dorm")){
				System.out.println();
			}
			String[] arr = str.split("\\|");
